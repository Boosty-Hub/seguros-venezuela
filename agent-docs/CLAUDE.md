...
5. **Wizard `/first-run`**: paso Conectar (ya hecho) → paso Inicializar (pegas un Personal Access Token `sbp_...` de `supabase.com/dashboard/account/tokens`; el wizard aplica las migraciones + deploya las Edge Functions con progreso en vivo) → paso Crear usuario (email + contraseña; bloquea registros adicionales). Al terminar, entras **directo al dashboard** (`/inbox`). **Acá el onboarding crea/provisiona todo el proyecto solo.**
6. **Wizard de provisioning (panel lateral, NO bloqueante)**: Anthropic credentials + Managed Agent + Verticales (opcional) + Memory Stores + Kommo. Vive como un drawer derecho SOBRE el dashboard (`web/src/app/(dashboard)/setup-drawer.tsx`), se auto-abre 1x por sesión si falta config, se puede cerrar/reabrir (botón flotante o `/setup`, que redirige a `/inbox?setup=open`). Idempotente. Estado computado en `web/src/lib/setup-state.ts` (DB-only).
7. **`/agent`**: editas voz/identidad/branding cuando quieras; al guardar sincroniza el system prompt con Anthropic (sube versión).

> **Guía paso-a-paso completa** (para humanos, con cada wizard detallado): `README.md` (Pasos 1-7) y `SETUP-WITH-CLAUDE.md` (Fase 4 deploy Netlify/Vercel · Fase 5 env vars + `/first-run`). Mantén estas tres en sync si cambia el flujo.
...
5. **Migraciones con `${SUPABASE_URL}` placeholder.** Las migraciones que crean cron jobs (0006, 0007, 0009, 0010, 0011, 0013) usan `'${SUPABASE_URL}/functions/v1/<fn>'` en lugar de URLs hardcoded. La sustitución ocurre **en runtime** dentro de `web/src/app/api/provision/migrate/route.ts` antes de ejecutar cada SQL. El placeholder viaja intacto en los archivos `.sql` y en el archivo generado `migrations.generated.ts`. **No reemplazar el placeholder con la URL real en los archivos SQL** — deja el placeholder para que el repo siga siendo reusable.

6. **El webhook tiene que devolver rápido y NO quedar enganchado al downstream — si no, Kommo lo deshabilita.** Kommo apaga un webhook cuyo endpoint le falla de forma sostenida, y no avisa a nadie: en el deployment de KIA eso dejó el sistema mudo 6 días seguidos (jul 30–ago 4 de 2026) y otros 4 (ago 7–ago 10), con 219 mensajes de clientes que nunca entraron al sistema. La causa era estructural: `kommo-webhook` disparaba **un `process-inbound` por cada webhook** y lo sostenía con `waitUntil` hasta que terminaba el lote entero (MAX_BATCH payloads × clasificador + API de Kommo = minutos), así que el worker del webhook vivía tanto como el downstream; bajo ráfaga (43.989 webhooks en un día) se agota el pool → 5xx → Kommo desconecta. Reglas que **no hay que revertir**:
   - `process-inbound` responde **202 al toque** y drena en `waitUntil` (antes hacía todo dentro del request handler).
   - `kommo-webhook` **coalesce** el trigger (máx. 1 cada 2s por instancia) y **solo dispara si el payload es accionable** (`message.add` / `leads.add`). `leads.update` y `leads.status` son ~**64% del tráfico** y `processPayload` los ignora — se encolan igual y el sweep de pg_cron los cierra. `isActionable` es **fail-open**: ante una forma que no sabe inspeccionar, dispara.
   - `claim_inbound_batch` tiene **reaper**: recupera los `processing` huérfanos de entre 10min y 24h (10min > el wall clock máximo de una Edge Function ⇒ están muertos con certeza) y marca `failed` los de +24h en vez de replayarlos — reprocesar un mensaje viejo le mandaría una respuesta a una conversación fría, porque `messages.created_at` se reescribe a `now()` y la ventana de frescura (`answer_max_age_hours`) ya no lo filtra. Sin el reaper, un worker muerto dejaba la fila invisible **para siempre**.
   - `upsertLead` reintenta ante **23505**: el webhook y el sweep de pg_cron corren en paralelo y el select-then-insert se pisa; el error mataba la fila entera de la cola con TODOS sus mensajes.
   - Red de seguridad: la alerta **`inbound_silence`** (`alerts-scan`) salta con cero webhooks en 90 min de horario laboral. Es lo único que convierte un apagón de días en un aviso.
...
- **Dreams**: `dreams-run` analiza conversaciones (24h/7d), Sonnet destila learnings y los escribe como `.md` en `<master>/dreams/`. **El agente ya NO lee `/dreams/` por filesystem** (con muchos archivos activos eso costaba listado + lecturas + turnos extra en CADA respuesta): tras cada corrida, `dreams-run` consolida TODOS los dreams activos en un **digest rodante** (`rebuildDigest`, ≤900 palabras, parte del digest anterior + activos) guardado en `runtime_config.DREAMS_DIGEST`, y `generate-response` lo inyecta al contexto de cada sesión como bloque `aprendizajes_del_operador` (**prioridad mayor que la voz base**, TTL 60s, sin re-sync). Las mutaciones del dashboard (aprobar/borrar/importar) disparan `dreams-run {digest_only:true}` fire-and-forget (`triggerDigestRebuild`, `lib/memory-list.ts`) para que el cambio llegue al agente en <60s. Cota de activos: `DREAMS_MAX_ACTIVE` (default 60) — el exceso más viejo se archiva a `/dreams-archive/` (fuera del dashboard y de futuras consolidaciones; su esencia persiste en el digest rodante, en tandas de `ARCHIVE_BATCH=50` por corrida para no exceder los recursos del worker). Cada learning lleva `severity` (sugerencia|advertencia|error, codificada también en el filename como `sug|adv|err`) y la política `runtime_config.DREAMS_AUTO_ACTIVATE` decide qué se activa solo: `all` (default), `error` (solo errores se auto-activan = autocorrección; el resto espera aprobación) o `none`. Lo no activado va a `/dreams-pending/` (no entra al digest) y se aprueba/descarta desde `/dreams`. Todo dream `error` genera una alerta (`kind=dream_error`). Borrar un activo lo saca del digest en el próximo rebuild (<60s).
