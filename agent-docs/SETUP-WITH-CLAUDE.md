...
>
> **Si eres un humano**: la primera fase de abajo (Fase 0) es crear el repo nuevo. Después abres ESE repo (no este) con Claude Code y dile *"Sigue SETUP-WITH-CLAUDE.md"*. Claude te va a guiar hasta dejar el agente desplegado.
>
> **Si eres Claude leyendo esto**: tu primera responsabilidad es VERIFICAR que el usuario está trabajando en un repo nuevo, no en el template. Si está en `kommo-claude-agent-template`, haz Fase 0 con él/ella. Si ya está en su repo propio, sáltate Fase 0 y arranca en Fase 1. En cada fase: lee el **Objetivo**, haz las **Preguntas al usuario**, espera respuesta, ejecuta las **Acciones**, valida el **Checkpoint**, y solo entonces avanza. **No avances si una validación falló** — investiga la causa y resuelve con el usuario.
>
> Si Claude no tiene tools (Claude.ai web sin file access): lee las acciones y díctaselas al usuario.
...
## Fase 0a — ¿Tienes un workflow de n8n existente?

**Objetivo**: si el usuario ya tiene un agente en n8n y lo quiere migrar, extraer todo lo que se pueda automáticamente.

**Preguntas al usuario**:
1. "¿Tienes un workflow de n8n del agente que quieres migrar? Si sí, expórtalo desde n8n (botón ⋯ → Download) y pégame el JSON."

**Acciones**:
- **Si SÍ**: lee `agent/from-n8n.md` y aplícalo. Extrae operator name, system prompt, modelo, verticales, credenciales que aparezcan.
...
**Nota**: estos valores ya NO van en `.env.local`. Se ingresan directamente en el wizard `/setup` del dashboard después del deploy. Puedes anotarlos para tenerlos a mano.

**Checkpoint**: tienes anotados los datos de identidad. No hay nada que editar en el repo todavía.
...
1. Crea un proyecto Supabase: `supabase.com/dashboard` → "New project" → región más cercana.
2. Obtén estos valores (los vas a pegar en el host en Fase 3):
...
1. Obtienes `ANTHROPIC_API_KEY` de `console.anthropic.com/settings/keys` → "Create Key".
   Esta clave la ingresas en el wizard `/setup` después del deploy.

### 2.3 Kommo (puedes hacerlo después de Fase 4 si no lo tienes)

1. Crea una integración privada: Settings → Integrations → "Create integration" → tipo Private → scope `crm`.
2. Anota: `KOMMO_CLIENT_ID`, `KOMMO_CLIENT_SECRET`, `KOMMO_LONG_LIVED_TOKEN`, `KOMMO_SUBDOMAIN`, `KOMMO_API_DOMAIN`.
   Estos se ingresan en el wizard `/setup` → paso Kommo.

**Checkpoint**: tienes los valores anotados. NO se escriben en ningún archivo .env todavía.
...
1. "¿Tienes ya escrito en algún lado un system prompt o una guía de voz del operador? Pásamelo."
2. Si no: completa los marcadores `[REEMPLAZA ESTA SECCIÓN]` de `agent/system-prompt.example.md`. Mínimo:
...
**Checkpoint**: tienes un system prompt que el operador aprobaría. Lo vas a pegar en el wizard en Fase 5.
...
**Checkpoint**: tienes una URL pública (aunque el sitio aún no funcione).
...
3. Al crear el usuario, el wizard inicia sesión y te lleva directo al dashboard (`/inbox`). El onboarding de Anthropic + Kommo sigue como panel lateral (drawer), no bloqueante.
...
## Fase 6 — Wizard de provisioning (panel lateral, Anthropic + Kommo)

**Objetivo**: provisionar Managed Agent, Memory Stores y conexión Kommo desde el dashboard.

Ya dentro del dashboard, un **panel lateral derecho** ("Configurá tu agente") guía el onboarding. No es obligatorio ni bloquea el acceso: se abre solo la primera vez, se puede cerrar y reabrir (botón flotante abajo a la derecha, o visitando `/setup`), y el progreso persiste. Provisiona en orden:
...
2. **Managed Agent**: crea el Environment + Agent con el system prompt.
3. **Verticales** (opcional): la IA sugiere categorías de mensaje a partir del agente creado.
4. **Memory Stores**: crea los dos stores en Anthropic y guarda sus IDs.
5. **Kommo**: token, subdomain, API domain. Verifica contra la API y guarda.

Todo se escribe en `runtime_config` (DB). El wizard es idempotente — puedes re-correrlo sin duplicar nada.
...
| Webhook Kommo devuelve 401 | `verify_jwt=true` en la Edge Function | Las funciones se despliegan con `verify_jwt=false` vía el wizard. Si desplegaste por CLI, revisa `supabase/config.toml` |
