...
const CREATE_RULES = `Eres un experto en prompt engineering. Construye SOLO la parte de IDENTIDAD, VOZ y NEGOCIO de un agente conversacional de ventas/soporte que corre sobre Kommo CRM y responde a leads por mensajería (WhatsApp / Instagram / web).
...
- Responde ÚNICAMENTE con el system prompt. Nada de explicaciones.
- **Tuteo venezolano/latinoamericano estándar (tú, tienes, dices) — NUNCA voseo rioplatense (vos, tenés, decís)**, tanto en el texto que redactes como en las instrucciones de voz que definas para el agente.
...
const PLACEHOLDER_NOTE = `Refiérete a la marca como {{OPERATOR_NAME}} (no el nombre real). No toques otros placeholders ({{MASTER_PATH}}, {{LEADS_PATH}}, etc.). Usa siempre tuteo (tú/tienes), nunca voseo (vos/tenés).`;
...
const CONTINUE_RULES = `Continúa EXACTAMENTE desde donde termina el texto que te paso, SIN repetir ni una palabra y SIN agregar introducción. Mantén el formato markdown y el tuteo (nunca voseo). Termina con la línea exacta:
${END}`;
...
export async function POST(request: Request) {
  ...
  if (mode === "continue") {
    ...
    const ctx = titles.length
      ...
      : "";
    const titleHint = title ? `\n\nUsa EXACTAMENTE este encabezado para la sección: ## ${title}` : "";
