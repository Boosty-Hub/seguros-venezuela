...
- Si menciona herramientas del workflow n8n (HTTP nodes, Function nodes), describe la intención pero quita los detalles técnicos — el agente acá usa solo la tool `search_kb`.
...
**Buscar**: nodos `n8n-nodes-base.set` que definen variables como `operatorName`, `agentName`, `botName`. O dentro del system prompt: "Eres el asistente de X", "Eres X", etc.
