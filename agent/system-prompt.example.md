...
[OPCIONAL] Cada vertical en la tabla `verticals` tiene su propio `system_prompt` que se inyecta antes del mensaje del lead. Acá puedes poner reglas que apliquen a TODAS las verticales (lo específico va en cada row de `verticals`).
